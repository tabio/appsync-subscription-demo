...
export class AppsyncSubscriptionDemoStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    // ---> AppSync
    // AppSyncの認証Lambda
    const appSyncAuthLambda = new lambdaNodejs.NodejsFunction(
      this,
      "appSyncAuthLambda",
      {
    ...
    const appsyncApi = new appsync.GraphqlApi(
      this,
      "AppSyncSubscriptionDemoAppSyncApi",
      {
    ...
    dataSource.createResolver("createNotificationMutation", {
      typeName: "Mutation",
      fieldName: "createNotification",
      requestMappingTemplate: appsync.MappingTemplate.fromFile(
        "appsync/mapping-templates/create-notification-request.vtl",
      ),
      responseMappingTemplate: appsync.MappingTemplate.fromFile(
        "appsync/mapping-templates/create-notification-response.vtl",
      ),
    });

    // AppSyncにIAM認証でMutationを実行するLambdaをNodejsFunctionで作成
    const createNotificationLambda = new lambdaNodejs.NodejsFunction(
      this,
    ...
    createNotificationLambda.addToRolePolicy(
